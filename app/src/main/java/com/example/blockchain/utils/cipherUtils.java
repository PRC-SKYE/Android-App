...
public class cipherUtils {
    private static final String PASSWORD = "THor HAV HAM35";
    ...
    public static String encryptIt(@NonNull String value) {
        ...
        try {
            ...
            return android.util.Base64.encodeToString(cipher.doFinal(clearText), Base64.DEFAULT);
    ...
    public static String decryptIt(@NonNull String value) {
        try {
            ...
            SecretKeyFactory keyFactory = SecretKeyFactory.getInstance(ALGORITHM);
            SecretKey key = keyFactory.generateSecret(keySpec);
            byte[] encryptedPwsBytes = Base64.decode(value, Base64.DEFAULT);
            Cipher cipher = Cipher.getInstance(ALGORITHM);
            cipher.init(Cipher.DECRYPT_MODE, key);
            byte[] decryptedValueBytes = (cipher.doFinal(encryptedPwsBytes));
